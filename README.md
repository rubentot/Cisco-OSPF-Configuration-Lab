...
Router ID selection using loopbacks
OSPF network advertisement and adjacency formation
Cost calculation and manipulation for path selection/load balancing
Passive interfaces for security
Default route propagation
Multi-area OSPF with ABRs and inter-area routes
Route summarization to reduce table size
DR/BDR election on broadcast networks

Image References:
images/topology-r1-r5.png (Main topology with R1-R5, PCs, switches, Internet)
images/topology-multi-area.png (Multi-area layout)
images/topology-r6-r9.png (Ethernet segment for DR/BDR)
Real-World OSPF Usage (2025)
...
RankUsage ScenarioWhy OSPF?1Enterprise campuses/multi-vendor networksOpen standard, hierarchical design with areas for scalability2Data center underlays (with EVPN/BGP overlays)Fast convergence, link-state reliability3Service provider cores (alongside IS-IS)Multi-area support for large topologies4SD-WAN fabricsDynamic path selection based on cost/metrics5Lab/education environmentsTeaches advanced routing concepts like summarization and DR election
...
textinterface FastEthernet1/1
 ip ospf cost 1500
Default Route Injection
textip route 0.0.0.0 0.0.0.0 203.0.113.2
router ospf 1
...
A: Uses wildcard masks for precise advertisement.textrouter ospf 1
 network 10.0.0.0 0.255.255.255 area 0
 network 192.168.0.0 0.0.0.255 area 0
Q3: Expected Router ID on R1?
A: Loopback: 192.168.0.1 (highest loopback preferred). Verified with show ip protocols.textR1#sh ip protocols 
...
A:show ip ospf neighbor shows FULL states with neighbors.textR1#show ip ospf neighbor 
...
A: All 10.x and loopbacks present as O routes. Example output from R1 shown in lab.textR1#sh ip route 
...
A:auto-cost reference-bandwidth 100000 on all routers.textR1(config)#router ospf 1 
R1(config-router)#auto-cost reference-bandwidth 100000

OSPF Cost

Q7: Cost on FastEthernet links?
A: 1000 (100000 / 100). Verified with show ip ospf interface.textR1#show ip ospf interface FastEthernet 0/0 
...
Before:textR1#sh ip route 
O 10.1.2.0/24 [110/3] via 10.0.3.2, 00:02:51, FastEthernet1/1After:textR1#sh ip route 
O 10.1.2.0/24 [110/3000] via 10.0.3.2, 00:01:04, FastEthernet1/1
Q9: Path in table for 10.1.2.0/24?
A: Via R5 (10.0.3.2) as lowest cost.textR1#sh ip route 
O 10.1.2.0/24 [110/3000] via 10.0.3.2, 00:01:04, FastEthernet1/1
Q10: Change this so that traffic from R1 to 10.1.2.0/24 will be load balanced via both R2 and R5.
A: Since we changed the reference bandwidth, all interfaces have a cost of 1000. The current path from R1 > R5 > R4 has a cost of 3000 (the cost of the destination interface itself is also counted in the total cost). The path from R1 > R2 > R3 > R4 has a cost of 4000. The easiest way to configure both paths to have the same cost is to configure the links from R1 > R5 and R5 > R4 to have a cost of 1500 each. (R1 > R5 = 1500, plus R5 > R4 = 1500, plus cost of 10.1.2.0/24 interface on R4 = 1000. Total = 4000).textR1(config)#int f1/1 
...
A: Two routes shown in table.textR1#sh ip route 
...
A: The 203.0.113.0/24 network must be added to the OSPF process on R4, and interface FastEthernet 1/1 facing the service provider configured as a passive interface to avoid sending out internal network information.textR4(config)#router ospf 1 
...
A: Route present in table.textR1#sh ip route 
...
A: R3 and R4 require no change as all their interfaces are already in Area 0. R1’s interfaces need to be reconfigured to be in Area 1 rather than Area 0.textR1(config)#router ospf 1 
R1(config-router)#network 10.0.0.0 0.255.255.255 area 1 
R1(config-router)#network 192.168.0.0 0.0.0.255 area 1 
R1#copy running-config startup-config 
R1#reloadR2 interface FastEthernet 0/1 should remain in Area 0. FastEthernet 0/0 needs to be reconfigured to be in Area 1. I used a 10.0.0.0/8 network statement originally so I need to remove that and add more granular statements.textR2(config)#router ospf 1 
...
R2#reloadR5 interface FastEthernet 0/0 should remain in Area 0. FastEthernet 0/1 needs to be reconfigured to be in Area 1.textR5(config)#router ospf 1 
...
A:show ip ospf interface confirms.textR2#show ip ospf interface 
...
A:textR1#show ip ospf neighbor 
...
A: The networks beyond R2 and R5 will appear as Inter Area routes (apart from the default route which will appear as an external route as it was redistributed into OSPF).textR1#sh ip route 
10.0.0.0/8 is variably subnetted, 12 subnets, 2 masks 
C 10.0.0.0/24 is directly connected, FastEthernet0/0 
...
A: We configured the link from R1 to R5 to have a higher cost than the link from R1 to R2 earlier.textR1#sh run | begin interface FastEthernet1/1 
...
A: OSPF priority has not been set so all routers will have the default of 1. R9 and R8 will be elected as the DR and BDR respectively because they have the highest Router IDs (because they have the highest IP addresses on their loopback interfaces).textR6#show ip ospf interface FastEthernet 0/0 
FastEthernet0/0 is up, line protocol is up 
...
A: Configure a higher OSPF priority on R6.textinterface FastEthernet0/0 
 ip ospf priority 100 
end 
clear ip ospf process
Q31: Verify R6 is the Designated Router.
A:textR6#show ip ospf interface FastEthernet 0/0 
FastEthernet0/0 is up, line protocol is up 
Internet address is 172.16.0.6/24, Area 0 
Process ID 1, Router ID 192.168.0.6, Network Type BROADCAST, Cost: 1000 
